    ...
    this._api.restfulPost( { create, params }, 'Sf/updateOportunity' )
                .subscribe( res => {
                  ...
                  for( let f in res['data'] ){
                    let field = res['data'][f]
                    if( typeof field == 'boolean' ){
                      updatedFields[f] = field
                      okResult = true
                    }
                  }
                  ...
                  if( okResult ){
                    ...
                    if( res['data']['opportunityFields'] && typeof res['data']['opportunityFields'] == 'object' ){
                      for( let f in res['data']['opportunityFields'] ){
                        let field = res['data']['opportunityFields'][f]
                        if( this.opCtrlProps[f] &&  this.opportunityForm.get(f) ){

                          let isDate = false
                          if( typeof field == 'string' ){
                            let matches = f.match(/^Fecha.*$/);
                            isDate = matches ? true : false;

                            if( isDate ){
                              field = moment(field)
                              this.dateChange.emit( { field: f, value: field } )
                            }
                          }

                          this.opportunityForm.get(f).setValue( field )
                        }
                      }
                    }


                    Swal.fire('Guardado', res['msg'] + ' ' + JSON.stringify(updatedFields), 'success')
                  }else{

                    let msg = ''
                    if( res['data'][0] && res['data'][0]['errorCode'] ){
                      msg = res['data'][0]['message']
                    }

                    Swal.fire('Error', 'No se actualizó el registro.' + ' ' + msg, 'error')
                    if( create ){
